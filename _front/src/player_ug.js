...
const ducksoupListener = (message) => {
  ...
  if (kind === "track") {
    ...
    if (track.kind === "video") {
      ...
      document.querySelector(".ducksoup-container .connecting").classList.add("d-none");
      document.querySelector(".ducksoup-container .connected").classList.remove("d-none");
      // append
      mountEl.appendChild(el);
      // unmute
      if (state.joinedType === "reconnection") {
    ...
    var proposerText = document.getElementById("proposer_text");
    var responderText = document.getElementById("responder_text");
    if (proposerText) proposerText.remove();
    if (responderText) responderText.remove();
